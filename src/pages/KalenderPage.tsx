    ...
    if (editingPost) {
      ...
      if (!error) {
        setPosts(prev => prev.map(p =>
          p.id === editingPost.id
            ? { ...p, title: formTitle || null, content: formContent, platform: formPlatform, status: formStatus }
            : p
        ))
        showToast('Sparat ✓')
        setEditingPost(null)
    ...
    <Layout>
      <div className="max-w-3xl mx-auto px-5 py-10">
        ...
        <div className="flex items-center justify-between mb-5">
          <button onClick={prevMonth} className="w-9 h-9 rounded-full border border-outline-variant/40 flex items-center justify-center hover:bg-surface hover:border-primary/40 transition-all">
            <span className="material-symbols-outlined text-[18px]">chevron_left</span>
          </button>
          <h2 className="serif-headline text-2xl italic">{MONTHS_SV[month]} {year}</h2>
          <button onClick={nextMonth} className="w-9 h-9 rounded-full border border-outline-variant/40 flex items-center justify-center hover:bg-surface hover:border-primary/40 transition-all">
